...
namespace Lab2
{
    class Program
    {
        static void Main(string[] args)
        {
             ...
             * "And makes " -> [15]
             * "Is sicklied " -> [0]
             * "For who would " -> [GOOGLE]
             * "For who would bear the whips and scorns of time,"
             */

            List<byte[]> ciphers = new List<byte[]>(input.Length);
            for (int i = 0; i < input.Length; i++)
            {
                ciphers.Add(XSalsa20Cracker.HexStringToByteArray(input[i]));
            }
            string word = "For who would bear the whips and scorns of time,";

            XSalsa20Cracker.Analyze(ciphers, word);
        }
    }
}
